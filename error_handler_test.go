...
func TestErrorHandler_IndependentBudget(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		runner := mustRunner(t, Config{Timeout: time.Second, ErrorHandlerTimeout: 3 * time.Second},
			func(ctx context.Context) (int, error) { <-ctx.Done(); return 7, ctx.Err() },
...
func TestErrorHandler_FailureDoesNotReplaceWork(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		r := mustRunner(t, Config{Timeout: time.Second, ErrorHandlerTimeout: 2 * time.Second},
			func(context.Context) (int, error) { return 2, workErr },
...
func TestErrorHandler_ReportsSwallowedDeadline(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		r := mustRunner(t, Config{ErrorHandlerTimeout: time.Second}, func(context.Context) (int, error) { return 0, errors.New("work") },
...
func TestErrorHandler_NotCalledWithoutReturnedError(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		calls := 0
		r := mustRunner(t, Config{Timeout: time.Second}, func(context.Context) (int, error) { time.Sleep(2 * time.Second); return 0, nil },
			WithErrorHandler(func(context.Context, error) error { calls++; return nil }))
		result := r.Run(context.Background())
		if calls != 0 || result.Outcome != OutcomeTimeout {
			t.Fatalf("calls=%d result=%+v", calls, result)
		}
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
		result = r.Run(ctx)
		if calls != 0 || !result.Start.IsZero() {
...
func TestErrorHandler_ParentCancellationDoesNotCancelDelivery(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	r := mustRunner(t, Config{}, func(context.Context) (int, error) { cancel(); return 0, context.Canceled },
...
func TestErrorHandler_Configuration(t *testing.T) {
	if _, err := MakeRunner(Config{ErrorHandlerTimeout: -time.Second}, func(context.Context) (int, error) { return 0, nil }); err == nil {
		t.Fatal("accepted negative timeout")
	}
	r := mustRunner(t, Config{}, func(context.Context) (int, error) { return 0, errors.New("work") }, WithErrorHandler(func(context.Context, error) error { t.Fatal("disabled handler ran"); return nil }), WithErrorHandler(nil))
