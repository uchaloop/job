...
func runOnce() int {
	...
	// The attempt runs under the signal: an evicted pod stops the work instead
	// of being killed in the middle of a batch.
	workCtx, cancelWork := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	result := runner.Run(workCtx)
	cancelWork()

	// The observation gets a bounded context of its own, because the work
	// context is cancelled by now.
